...
async function inspectAcc() {
  const { data: tours } = await supabase.from('tours').select('*').order('updated_at', { ascending: false }).limit(1);
  if (!tours || tours.length === 0) return;
  const tour = tours[0];
  const accommodations = tour.planner_data?.accommodations || [];
  
  console.log('Accommodations in planner_data:');
  accommodations.forEach(a => {
    console.log(`Night ${a.nightIndex}: Hotel Name: ${a.hotelName}, Hotel ID: ${a.hotelId}, Room ID: ${a.roomId}, Room Name: ${a.roomName}`);
  });
}

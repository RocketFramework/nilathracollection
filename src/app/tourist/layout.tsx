...
export default function TouristLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            ...
            if (user && user.email) {
                ...
                setUserEmail("Tourist User");
                // If we want to strictly protect, we could router.push('/tourist/login') here if not on login page
                if (!pathname?.includes('/tourist/login')) {
    ...
    return (
        <div className="min-h-screen bg-[#F9FAFB] flex flex-col">
                            ...
                            <div className="relative">
                                ...
                                >
                                    <div className="w-9 h-9 rounded-full bg-brand-gold/10 text-brand-gold border border-brand-gold/20 flex items-center justify-center font-bold">
                                        {userEmail !== "Loading..." && userEmail !== "Tourist User" ? userEmail.charAt(0).toUpperCase() : "T"}
                                    </div>
                                    <span className="hidden sm:block text-sm font-medium text-neutral-700">{userEmail}</span>
            ...
            {/* Main Content Area */}
            <main className={`flex-1 w-full ${!pathname?.includes('/tourist/login') ? 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8' : ''}`}>
                {children}
            </main>
        </div>
    );

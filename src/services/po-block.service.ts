...
export class POBlockService {
    ...
    static async initializeDefaultBlocks(tourId: string): Promise<POBlock[]> {
        ...
        const { data: activities, error: actErr } = await adminSupabase
            .from('daily_activities')
            .select('*, tour_itineraries(day_number, date)')
            .eq('tour_id', tourId);
        ...
        const sleepActivities = activities.filter(act => act.activity_type === 'sleep');
        ...
        // 2. TRAVEL: select only travel, group by transport_id
        const travelActivities = activities.filter(act => act.activity_type === 'travel');
        const travelGroups = new Map<string | null, any[]>();
        travelActivities.forEach(act => {
            const transportId = act.transport_id || null;
        ...
        const transportIds = Array.from(travelGroups.keys()).filter(Boolean) as string[];
        let transportVendors: { id: string, name: string }[] = [];
        if (transportIds.length > 0) {
            const { data: vendorData } = await adminSupabase
                .from('vendors')
                .select('id, name')
                .in('id', transportIds);
            transportVendors = vendorData || [];
        }

        for (const [transportId, group] of travelGroups.entries()) {
            const vendorObj = transportVendors.find(v => v.id === transportId);
            const vendorName = vendorObj ? vendorObj.name : 'Unassigned Transport';
        ...
        const mealActivities = activities.filter(act => act.activity_type === 'meal');
        ...
        const regularActivities = activities.filter(act => act.activity_type === 'activity');
